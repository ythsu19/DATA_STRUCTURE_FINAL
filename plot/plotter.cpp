...
// 這是嵌入在 C++ 中的 Python 腳本
const string python_script = R"(
import pandas as pd
...
int main() {
    // 1. 檢查 CSV 是否存在
    ifstream file("../benchmark/benchmark_result.csv");
    if (!file.good()) {
        cerr << "Error: 'benchmark_result.csv' not found. Please run benchmark first." << endl;
        return 1;
    }
    file.close();

    // 2. 產生 Python 腳本檔案
    ofstream py_file("plot_chart.py");
    py_file << python_script;
    py_file.close();

    // 3. 呼叫系統 Python 執行
    // 根據你的環境，可能是 python 或 python3
    cout << "Executing Python script to generate plot..." << endl;
    int ret = system("python3 plot_chart.py");
    
    if (ret != 0) {
        // 如果 python3 失敗，嘗試 python
        ret = system("python plot_chart.py");
    }

    if (ret == 0) {
        cout << "Plotting finished! Open 'benchmark_analysis.png' to view result." << endl;
        // 刪除臨時腳本 (可選)
        // system("rm plot_chart.py");
    } else {
        cerr << "Failed to run python script. Make sure you have python, pandas and matplotlib installed." << endl;
        cerr << "Try running: pip install pandas matplotlib" << endl;
    }
